...
export const ContactPage: React.FC<ContactPageProps> = ({ navigate }) => {
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    if (!bookingName.trim() || !bookingPhone.trim() || !bookingDate) {
      speakText('Please fill in your name, phone number, and preferred date.');
      return;
    }
    setBookingSuccess(true);
    speakText('Your introductory telephone call request has been received. Our UK team will call you at your selected time.');
  };

  const handleMessageSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!msgName.trim() || (!msgEmail.trim() && !msgPhone.trim()) || !msgContent.trim()) {
      speakText('Please provide your name, contact details, and message.');
      return;
    }
    setMsgSuccess(true);
    speakText('Your message has been sent. Our British support desk will respond within one business day.');
  };
