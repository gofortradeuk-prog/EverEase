...
export const AuthPage: React.FC<AuthPageProps> = ({ 
  ...
  const handleGetStartedSubmit = async (e: React.FormEvent) => {
    ...
    speakText('Generating your Unique ID Code, temporary password, and preparing your Stripe invoice with BACS Direct Debit and Card options.');
    ...
    try {
      ...
      speakText(
        `Success! Your Unique ID Code is ${data.uniqueMemberId}. Your temporary password and Stripe invoice have been sent to ${email}. You can pay via BACS Direct Debit or Card.`
      );
    } catch (err: any) {
  ...
  const handleImmediateLoginWithTempCredentials = async () => {
    ...
    setIsSubmitting(true);
    speakText('Logging into your new EverEase dashboard.');
    try {
  ...
  const copyToClipboard = (text: string, type: 'id' | 'pass') => {
    ...
    if (type === 'id') {
      setCopiedId(true);
      speakText(`Copied Unique ID Code: ${text}`);
      setTimeout(() => setCopiedId(false), 2500);
    } else {
      setCopiedPass(true);
      speakText('Copied temporary password to clipboard.');
        ...
        {navigate && (
          <div className="flex items-center justify-between">
            <button
              type="button"
              onClick={() => {
                speakText('Returning to public website');
                navigate('/');
              }}
